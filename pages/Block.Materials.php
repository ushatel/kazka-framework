<?php
  ...
  include_once("Block.Materials_Local.php");
  ...
  include_once("includes/common/Main.Enumerator.php");
	 ...
	 * class Materials
	 *
	 * Клас роботи із матеріалами
	 *
	 * @package Blocks.pkg
	 */

	class BlockMaterials extends CommonBlock
	{
		public $page = NULL;
		...
		private $fieldsArray = array("materials_group" => false, 
					     ...
					     "is_public" => false, 
					     "materials_unique_name" => false,
					     "materials_divisibility_id" => false,
					     "country_name_field" => false,
					     "materials_user_quantity" => false,
					     "materials_users_divisibility_id" => false,
					     "request_time" => false,
						 "groupsName" => false,
						 "parentGroup" => false
						 );
		...
		public $BlockMode = SELECT_MATERIALS ; // SELECT_MATERIALS = renders the form to select materials,
		...
		public $materialsWindow = 15;
		public $materialsOffset = 0;
		...
		{
			$this->localizator = new BlockMaterials_Local();
			...
			{
				$this->linkForListing = Request::GetRoot()."/materials/";
			}
		...
		{
			return array (sha1("SubMitNewMateRialtODb"), sha1("SavEeXisTingMateRial"), 
		...
		public function ValidateSupplierForm()
		{
			...
			$this->materials->user_supplier_companies_id = (float)Security::DecodeUrlData(StaticDatabase::CleanupTheField($_POST['companiesId']));
			$this->materials->user_supplier_companies_name = StaticDatabase::CleanupTheField(substr($_POST["companiesInput"],0,200));

			if($this->materials->user_supplier_companies_id == 0)
			{
				$this->fieldsArray["materials_companies_id"] = "<:=Validate_Company_Id_Error=:>";
				$this->isValidForm = false;
			}
			else
			{
			...
			if($this->materials->user_divisibility_id == 0 || $this->materials->user_divisibility_id > 0 && !$this->materials->ValidateUserDivisibility())
			{
