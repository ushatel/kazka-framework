<?php
  ...
  if (@preg_match("/pages/i", $_SERVER['PHP_SELF'])) 
	...
	include_once("pages/Page.Services_Local.php");
	...
	include_once("Block.Services.php");
	include_once("Block.Search.php");
	include_once("Block.Materials.php");
	
	class Services 	extends CommonPage
	{
		...
		{
			$this->localizator = new Services_Local();
		...
		public function PageInit()
		{
			...
			if(!$this->isAjaxRequest)
			{
				...
				$blockService->BlockInit();
				$blockService->BlockMode = SELECT_SERVICES;

				$this->blocks['BLOCK_SERVICES'] = $blockService;

				$this->pagesMarkup .= "<:=BLOCK_SERVICES=:></div>";
			}

			/*
			if(!$this->isAjaxRequest)
			{
				$this->pagesMarkup .= "<div id='ServicesBlock'>";
*/
